 ...
 * HTML-escaped.
 */
public final class ExtraSimpleHtmlSanitizer implements HtmlSanitizer {

  private static final ExtraSimpleHtmlSanitizer INSTANCE = new ExtraSimpleHtmlSanitizer();
   ...
   * @return the instance
   */
  public static ExtraSimpleHtmlSanitizer getInstance() {
    return INSTANCE;
  }
  ...
  private static String simpleSanitize(final String text) {
    ...
    for (final String segment : text.split("<", -1)) {
      if (firstSegment) {
        /*
         * the first segment is never part of a valid tag; note that if the input string starts with a tag, we will get an empty segment at the
         * beginning.
         */
        firstSegment = false;
      ...
      String tag = null;
      String tagWhole = null;
      boolean isValidTag = false;
      if (tagEnd > 0) {
        if (segment.charAt(0) == '/') {
          tagStart = 1;
        }
        tagWhole = segment.substring(tagStart, tagEnd);
        if (tagWhole.indexOf(" ") > -1) {
          tag = tagWhole.substring(0, tagWhole.indexOf(" "));
        } else {
          tag = tagWhole;
        }
      ...
      if (isValidTag) {
        ...
        sanitized.append(tagWhole).append('>');
  ...
  private ExtraSimpleHtmlSanitizer() {
  }
